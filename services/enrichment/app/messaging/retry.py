...
def retry_delay_sec(retry_count: int, base: float, cap: float) -> float:
    """Exponential backoff for retry_count >= 1, capped at ``cap`` seconds."""
    return float(min(cap, base * (2 ** max(retry_count - 1, 0))))


class EnrichmentRetryPublisher:
    """Publish failed work into the TTL retry queue (no in-process sleep)."""
    ...
    async def publish_retry(
        ...
        *,
        ...
        await self._exchange.publish(
            ...
            routing_key=self._settings.routing_key_enrichment_retry,
            mandatory=True,
        )
        self._metrics.inc(
            METRIC_RETRY_PUBLISHED,
        ...
        logger.warning(
            "Scheduled enrichment retry_count=%s delay_sec=%.2f",
            retry_count,
            delay_sec,
        )
